 ...
 */
class BlendGestureImageView2 @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : DPTransformImageView(context, attrs, defStyle) {
    companion object {
        // 定义误差容忍度
    ...
    init {
        ...
        viewTreeObserver.addOnGlobalLayoutListener(object : OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                ...
                this@BlendGestureImageView2.baseHeight = height
                this@BlendGestureImageView2.baseMarginTop = (this@BlendGestureImageView2.layoutParams as ViewGroup.MarginLayoutParams).topMargin
            }
        })
    }
...
//        canvas?.restoreToCount(saveCount!!)
        ...
        gradientPaint.shader = shader
        // 绘制渐变矩形到临时画布上
        tempCanvas.drawRect(0f, gradientHeight, width.toFloat(), 0f, gradientPaint)
        // 将临时位图绘制到实际画布上
        canvas?.drawBitmap(bitmap, 0f, 0f, paint)
        ...
        private fun handleAxisTranslation(mAxis: Axis, distance: Float) {
            ...
            if (axis < hasVibratedTranslate.size && hasVibratedTranslate[axis]) {
                ...
                if (isAngleCloseToOrthogonal) {
                    ...
                    when (mAxis) {
                        Axis.X -> {
                            imageViewValue = this@BlendGestureImageView2.width
                            bitmapStartValue = currentBitmapRect.left
                            bitmapEndValue = currentBitmapRect.right
                        }

                        Axis.Y -> {
                            imageViewValue = this@BlendGestureImageView2.height
