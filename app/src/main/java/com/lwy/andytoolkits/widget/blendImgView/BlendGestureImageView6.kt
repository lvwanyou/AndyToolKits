 ...
 */
class BlendGestureImageView6 @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : DPTransformImageView(context, attrs, defStyle) {
    companion object {
        // 定义误差容忍度
...
//        canvas?.restoreToCount(saveCount!!)
        ...
        val maxGradientHeight = 100 // 假设100为gradientHeight的最大值，可根据实际情况调整
        ...
        val shader2 = LinearGradient(
            0f, 0f, // 起始点
            gradientHeight.toFloat(), 0f, // 终止点（从底部到顶部50dp）
            endColor, // 终止颜色
            Color.WHITE, // 起始颜色
            Shader.TileMode.CLAMP
        )
        ...
        // 绘制渐变矩形到临时画布上
        tempCanvas.drawRect(0f, 0f, gradientHeight.toFloat(), height.toFloat(), gradientPaint)
    ...
    init {
        ...
        viewTreeObserver.addOnGlobalLayoutListener(object : OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                ...
                this@BlendGestureImageView6.baseHeight = height
                this@BlendGestureImageView6.baseMarginTop = (this@BlendGestureImageView6.layoutParams as ViewGroup.MarginLayoutParams).topMargin

                this@BlendGestureImageView6.baseWidth = width
                this@BlendGestureImageView6.baseMarginLeft = (this@BlendGestureImageView6.layoutParams as ViewGroup.MarginLayoutParams).leftMargin
            }
        })
    }
        ...
        private fun handleAxisTranslation(mAxis: Axis, distance: Float) {
            ...
            if (axis < hasVibratedTranslate.size && hasVibratedTranslate[axis]) {
                ...
                if (isAngleCloseToOrthogonal) {
                    ...
                    when (mAxis) {
                        Axis.X -> {
                            imageViewValue = this@BlendGestureImageView6.width
                            bitmapStartValue = currentBitmapRect.left
                            bitmapEndValue = currentBitmapRect.right
                        }

                        Axis.Y -> {
                            imageViewValue = this@BlendGestureImageView6.height
