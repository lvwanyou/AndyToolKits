 ...
 */
class BlendGestureImageView1 @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : DPTransformImageView(context, attrs, defStyle) {
    companion object {
        // 定义误差容忍度
    ...
    init {
        ...
        viewTreeObserver.addOnGlobalLayoutListener(object : OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                ...
                this@BlendGestureImageView1.baseHeight = height
            }
        })
    }

    private var isBlendModeOn = true
...
//            val tempCanvas2 = Canvas(bitmap)
//            drawable.setBounds(0, 0, width, height)
//            drawable.draw(tempCanvas2)
//            tempCanvas2?.concat(imageMatrix)
//

            // 计算终点透明度
            val gradientHeight = blendIntensity * density
            val maxGradientHeight = 100 * density // 假设300为gradientHeight的最大值，可根据实际情况调整
            val minAlpha = (255 * 0.7).toInt() // 设置最小透明度为128，即50%的透明度，防止太透明
            ...
            // 创建一个线性渐变
            // 这个设置意味着当使用 gradientPaint 画笔绘制渐变矩形时，实际上是在控件底部创建了一个渐变的遮罩效果。
            // 这个遮罩的透明度从完全不透明（白色）渐变到完全透明，因此底层的内容（即控件原本的内容）在这个渐变区域内会从完全可见渐变到完全不可见，从而实现了一个在控件底部实现渐变消失的视觉效果
            val shader = LinearGradient(
                0f, height - gradientHeight, // 起始点†®
                0f, height.toFloat(), // 终止点（从底部到顶部50dp）
                Color.WHITE, // 起始颜色
                endColor, // 终止颜色
            ...
            // 绘制渐变矩形到临时画布上
            tempCanvas.drawRect(0f, height - gradientHeight, width.toFloat(), height.toFloat(), gradientPaint)
        ...
        private fun handleAxisTranslation(mAxis: Axis, distance: Float) {
            ...
            if (axis < hasVibratedTranslate.size && hasVibratedTranslate[axis]) {
                ...
                if (isAngleCloseToOrthogonal) {
                    ...
                    when (mAxis) {
                        Axis.X -> {
                            imageViewValue = this@BlendGestureImageView1.width
                            bitmapStartValue = currentBitmapRect.left
                            bitmapEndValue = currentBitmapRect.right
                        }

                        Axis.Y -> {
                            imageViewValue = this@BlendGestureImageView1.height
