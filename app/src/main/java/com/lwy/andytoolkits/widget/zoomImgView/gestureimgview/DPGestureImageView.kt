 ...
 */
class DPGestureImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : DPTransformImageView(context, attrs, defStyle) {
    companion object {
        // 定义误差容忍度
        ...
        private fun handleAxisTranslation(mAxis: Axis, distance: Float) {
            ...
            if (axis < hasVibratedTranslate.size && hasVibratedTranslate[axis]) {
                ...
                if (isAngleCloseToOrthogonal) {
                    ...
                    when (mAxis) {
                        Axis.X -> {
                            imageViewValue = this@DPGestureImageView.width
                            bitmapStartValue = currentBitmapRect.left
                            bitmapEndValue = currentBitmapRect.right
                        }

                        Axis.Y -> {
                            imageViewValue = this@DPGestureImageView.height
