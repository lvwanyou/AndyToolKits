...
package com.lwy.andytoolkits.textureView;
 ...
 * A GLTextureView must be notified when to pause and resume rendering. GLTextureView clients
 * are required to call {@link #onPause()} when the activity stops and
 * {@link #onResume()} when the activity starts. These calls allow GLTextureView to
...
public class EGL14TextureView extends TextureView implements TextureView.SurfaceTextureListener {
    private final static String TAG = "Core-EGL14TextureView";
    ...
    private final static int EGL_GL_COLORSPACE_KHR = 0x309D;
    private final static int EGL_GL_COLORSPACE_DISPLAY_P3_PASSTHROUGH_EXT = 0x3490;
     ...
     * must call {@link #setRenderer} to register a renderer.
     */
    public EGL14TextureView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
     ...
     * 兼容广色域参数接口
     *
     * @param enabled 是否兼容广色域
     */
    public void setWideColorGamutCompatEnabled(boolean enabled) {
        if (enabled) {
            if (mEGLWindowSurfaceFactory == null || !(mEGLWindowSurfaceFactory instanceof WideColorGamutWindowSurfaceFactory)) {
                mEGLWindowSurfaceFactory = new WideColorGamutWindowSurfaceFactory();
            }
        } else {
     ...
     * this interface, and then call {@link EGL14TextureView#setRenderer} to
     ...
     * use the {@link EGL14TextureView#queueEvent(Runnable)} convenience method.
     * <p>
     ...
     * {@link EGL14TextureView#setEGLContextFactory(EGLContextFactory)}
     */
     ...
     * <p>
     * This interface must be implemented by clients wishing to call
     * {@link EGL14TextureView#setEGLWindowSurfaceFactory(EGLWindowSurfaceFactory)}
     */
    ...
    private static class WideColorGamutWindowSurfaceFactory implements EGLWindowSurfaceFactory {
     ...
     * <p>
     * This interface must be implemented by clients wishing to call
     * {@link EGL14TextureView#setEGLConfigChooser(EGLConfigChooser)}
     */
    public interface EGLConfigChooser {
         ...
         *
         * @param egl     the EGL14 for the current display.
    ...
    private static class EglHelper {
        public EglHelper(WeakReference<EGL14TextureView> glTextureViewWeakRef) {
...
//            mEgl = (EGL14) EGLContext.getEGL();
            ...
            EGL14TextureView view = mGLTextureViewWeakRef.get();
            if (view == null) {
                mEglConfig = null;
                mEglContext = null;
            } else {
...
//        }
    ...
    static class GLThread extends Thread {
        GLThread(WeakReference<EGL14TextureView> glTextureViewWeakRef) {
            super();
            mWidth = 0;
            mHeight = 0;
            mRequestRender = true;
...
//                                    createGlInterface = true;
                                ...
                                if (LOG_THREADS) {
                                    Log.i("GLThread", "before waiting tid=" + getId()
...
//                                Trace.traceEnd(Trace.TRACE_TAG_VIEW);
    ...
    private final WeakReference<EGL14TextureView> mThisWeakRef =
            new WeakReference<EGL14TextureView>(this);
